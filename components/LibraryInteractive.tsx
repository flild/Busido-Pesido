...
export interface ArticleData {
  ...
  title: string;
  summary: string;
  main_image?: string | null; // <-- Добавили новую колонку
  accent?: string; 
  textAccent?: string;
}
      ...
      <motion.div layout className="grid grid-cols-3 gap-5 tablet:grid-cols-2 mobile:grid-cols-1 items-stretch mb-10">
        <AnimatePresence mode="sync">
          {displayMaterials.map((m) => (
            ...
            >
              {/* Исправлена ссылка с /library на /blog */}
              <Link href={`/blog/${m.slug}`} className="group block h-full w-full outline-none">
                <TiltCard className="h-full w-full rounded-[28px] bg-white border border-forest/10 shadow-sm transition-all duration-500 relative flex flex-col overflow-hidden group-hover:-translate-y-1.5 group-hover:shadow-[0_25px_60px_rgba(30,43,14,0.08)] group-hover:border-forest/20">
                  ...
                  <div className={`absolute top-0 left-0 bottom-0 w-[6px] ${m.accent} opacity-80 group-hover:opacity-100 transition-opacity z-20`} />
                  
                  {/* Обложка (если есть) */}
                  {m.main_image && (
                    <div className="w-full h-[140px] relative shrink-0 overflow-hidden bg-snow border-b border-forest/5">
                      <Image 
                        src={m.main_image} 
                        alt={m.title} 
                        fill
                  ...
                  {/* Декоративный круг показываем, только если НЕТ обложки, чтобы не было визуального мусора */}
                  {!m.main_image && (
                  ...
                  <div className={`p-8 max-md:p-6 pl-[34px] flex flex-col h-full relative z-10 ${m.main_image ? 'pt-5' : ''}`}>
                    ...
                    <p className="text-coal/65 leading-relaxed text-[14px] flex-grow mb-6 line-clamp-3" title={m.summary}>
                      {m.summary}
                    </p>
