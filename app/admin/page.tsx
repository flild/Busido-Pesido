...
import { Users, FileText, TrendingUp, Activity, Eye, Calendar, ArrowRight, Star, BookOpen, ThumbsUp } from 'lucide-react';
...
export default async function AdminDashboard() {
  ...
  // Агрегируем метрики статей
  const metricsRow = db.prepare("SELECT SUM(views) as v, SUM(reads_count) as r FROM articles").get() as any;
  const totalViews = metricsRow?.v || 0;
  const totalReads = metricsRow?.r || 0;
  ...
  // ОБНОВЛЕНО: Топ статей считается по формуле (Дочитывания * 2 + Просмотры) и выводит новые поля
  const topArticles = db.prepare(`
    SELECT id, title, views, reads_count, likes, dislikes, is_premium, category 
    FROM articles 
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-5 gap-4 max-xl:grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1">
        ...
        {/* ОБНОВЛЕНО: Комбинированный виджет просмотров и дочитываний */}
        <div className="bg-white border border-forest/15 rounded-[20px] p-5 shadow-sm flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <div className="flex items-end gap-2 mb-1">
              <span className="text-2xl font-bold text-coal leading-none">{totalViews}</span>
              <span className="text-[13px] font-bold text-coal/40 mb-[3px]" title="Дочитывания">/ {totalReads}</span>
            </div>
            <div className="text-[11px] font-bold text-coal/50 uppercase tracking-wider">Просмотры / Дочитывания</div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 max-xl:grid-cols-1 gap-6">
        ...
        <div className="bg-white border border-forest/15 rounded-[24px] shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-0 flex-grow">
            {recentLeads.length === 0 ? (
              <div className="p-8 text-center text-coal/50 text-sm font-medium">Нет новых заявок. Можно выдохнуть.</div>
            ) : (
              <div className="flex flex-col">
                {recentLeads.map((lead, i) => (
                  <div key={lead.id} className={`p-4 flex items-center justify-between gap-4 hover:bg-snow transition-colors ${i !== recentLeads.length -1 ? 'border-b border-forest/5' : ''}`}>
                    <div className="overflow-hidden">
                      ...
                      <div className="text-xs font-medium text-coal/60 truncate mt-0.5">{lead.service}</div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-[11px] font-bold text-coal/40 mb-1">
        ...
        {/* ОБНОВЛЕНО: Таблица топа контента с расширенной статистикой */}
        <div className="bg-white border border-forest/15 rounded-[24px] shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-forest/15 flex justify-between items-center bg-snow/30">
            <h2 className="text-lg font-bold text-coal m-0 flex items-center gap-2">
          ...
          <div className="p-0 flex-grow">
            {topArticles.length === 0 ? (
              <div className="p-8 text-center text-coal/50 text-sm font-medium">Никто ничего не читал.</div>
            ) : (
              <table className="w-full text-left border-collapse min-w-[500px]">
                <thead>
                  <tr className="border-b border-forest/5 bg-snow/10">
                    <th className="p-3 pl-4 text-[11px] font-bold text-coal/40 uppercase tracking-wider">Статья</th>
                    <th className="p-3 text-[11px] font-bold text-coal/40 uppercase tracking-wider text-center">Просмотры</th>
                    <th className="p-3 text-[11px] font-bold text-coal/40 uppercase tracking-wider text-center">Дочитали</th>
                    <th className="p-3 pr-4 text-[11px] font-bold text-coal/40 uppercase tracking-wider text-center">Лайки</th>
                  </tr>
                </thead>
                <tbody>
                  {topArticles.map((art, i) => (
                    <tr key={art.id} className={`hover:bg-snow/50 transition-colors ${i !== topArticles.length -1 ? 'border-b border-forest/5' : ''}`}>
                      
                      <td className="p-4 pl-4 max-w-[220px]">
                        <div className="font-bold text-[14px] text-coal truncate mb-1" title={art.title}>
                          {art.title}
                        </div>
                        <div className="flex gap-2 items-center">
                          ...
                          <span className="text-[11px] font-medium text-coal/50 truncate">{art.category}</span>
                        </div>
                      </td>

                      <td className="p-4 text-center align-middle">
                        <div className="inline-flex items-center gap-1.5 font-bold text-coal">
                      ...
                      <td className="p-4 pr-4 text-center align-middle">
                        <div className="inline-flex items-center gap-1.5 font-bold text-matcha">
