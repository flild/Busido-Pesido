...
export async function saveSpecialist(formData: FormData, id?: number) {
  ...
  const city = formData.get('city')?.toString().trim(); // Добавили город
  ...
  // Жесткая валидация
  if (!name || !city || !role || !short_bio || !full_bio) {
    return { error: 'Заполните все обязательные поля, включая город' };
  }
  ...
  try {
    ...
    if (id) {
      db.prepare(`
        UPDATE specialists 
        SET name = ?, city = ?, role = ?, short_bio = ?, full_bio = ?, image_url = ?, is_main = ?, sort_order = ?
        WHERE id = ?
      `).run(name, city, role, short_bio, full_bio, image_url, is_main, sort_order, id);
    } else {
      db.prepare(`
        INSERT INTO specialists (name, city, role, short_bio, full_bio, image_url, is_main, sort_order)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(name, city, role, short_bio, full_bio, image_url, is_main, sort_order);
    }
  } catch (error: any) {
